...
class CuMemAllocator:
    ...
    def sleep(self, offload_tags: tuple[str, ...] | str | None = None) -> None:
        ...
        for ptr, data in self.pointer_to_data.items():
            ...
            # 这里调用python层free接口，同时保留地址信息
            # v = self.pointer_to_data[ptr]
            # handle = self._python_free_callback(ptr)
            # self.pointer_to_data[ptr] = v
    ...
    def wake_up(self, tags: list[str] | None = None) -> None:
        ...
        for ptr, data in self.pointer_to_data.items():
            if tags is None or data.tag in tags:
                handle = data.handle
                # 这里调用python层malloc接口，接口里面会刷新地址信息
                # self._python_malloc_callback(handle)
